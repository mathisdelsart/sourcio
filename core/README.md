...
| `config.py` | `Settings` — every env-driven setting, in one place. |
| `llm.py` | `get_llm(role)` — the model-agnostic factory (OpenAI / Anthropic / Groq / Ollama), the per-request visitor key, and the optional response cache. |
| `prompts.py` | Prompt fragments shared across the agent nodes, so the grounding rules are stated once. |
| `qdrant.py` | The shared Qdrant client and a bounded scroll helper, so no module builds its own. |
