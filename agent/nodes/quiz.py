...
_SYSTEM = (
    ...
    "- Refuse ONLY when the sources do not contain the TOPIC the quiz is about: the"
    " request asks to be quizzed on a specific concept, method or formula that the"
    " numbered sources never state (they merely share notation, mention a related"
    " idea, or are about a different part of the subject). Then reply with exactly"
    f" this sentence and nothing else: {REFUSAL}\n"
    "- When the request names no single topic (e.g. 'quiz me on the course' or"
    " 'covering both courses'), build questions spread across the retrieved"
    " sources; do not refuse.\n"
