...
_SYSTEM_HEAD = (
    ...
    "- Build an exercise ONLY on a concept, method or formula that the numbered "
    "sources actually STATE. The material may be of any kind: a worked problem "
    "when it is mathematical, or recall / short-answer / Q&A questions when it is "
    "factual, biographical or prose. Never refuse merely because the material is "
    "non-mathematical or has no formulas.\n"
    "- You are the judge of coverage, and the bar is the SPECIFIC thing the request "
    "asks to practise, not the broad topic. If the sources do not contain that "
    "exact concept, method or formula — even when they share notation, mention a "
    "related idea, or are about the same broad subject — reply with exactly this "
    f"sentence and nothing else: {REFUSAL}\n"
    "  Example: sources that define the Lorentz factor and time dilation do NOT "
    "cover an exercise on relativistic energy or momentum unless those formulas are "
    "themselves stated — refuse instead of importing them.\n"
    ...
    "exercise on material the sources do not contain just because some source was "
    "retrieved. When the material uses the course's notation, keep that notation.\n"
